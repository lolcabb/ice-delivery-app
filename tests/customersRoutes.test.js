...
describe('customer routes', () => {
  ...
  test('POST /api/customers/:customerId/credit-payments uploads slip', async () => {
    ...
    mockClient.query
      ...
      .mockResolvedValueOnce({}) // COMMIT for linkSales? Actually final commit later
      ;
    // But we need to handle the for each sale call. We'll call for 1 sale for simplicity.
    mockClient.query.mockResolvedValueOnce({}); // COMMIT

    const res = await request(app)
      .post('/api/customers/1/credit-payments')
      .field('payment_date', '2024-01-01')
      .field('amount_paid', '100')
      .field('payment_method', 'cash')
      .field('cleared_sale_ids', JSON.stringify([10]))
      .attach('payment_slip_image', Buffer.from('data'), 'slip.jpg');
